...
public function getStatistics($user_id = null, $role = null) {
    ...
    if ($role === 'demandeur') {
        ...
        SELECT 
            COUNT(*) AS total,
            SUM(CASE WHEN statut = 'validee' THEN 1 ELSE 0 END) AS a_traiter,
            SUM(CASE WHEN urgence = 'urgente' THEN 1 ELSE 0 END) AS urgentes,
            SUM(CASE WHEN statut = 'en_cours_de_traitement' THEN 1 ELSE 0 END) AS en_cours,
            SUM(CASE WHEN statut = 'traitee' THEN 1 ELSE 0 END) AS traitee
        FROM demandes
        ";
